...
import {
  ...
  DISASSEMBLY_LABELS,
  GENERAL_JUNK_ITEMS,
} from "../../../src/lib/pricing/general-junk";
import type {
  AccessType,
  DisassemblyType,
  GeneralJunkItemKey,
...
import { syncGeneralJunkRemovalOrderToJobber } from "../../_lib/orders/jobber-sync";
...
/**
 * POST /api/orders/general-junk-removal
 *
 ...
 * guarantee. See that file's comment for the full rationale.
 */
export async function onRequestPost(context: RequestContext): Promise<Response> {
  ...
  let record: GeneralJunkRemovalOrderRecord | null = suppliedOrderId
    ? await getGeneralJunkRemovalOrder(env.ORDERS_KV, suppliedOrderId)
    : null;
  ...
  if (!record) {
    ...
    if (!Array.isArray(payload.items)) {
      return jsonError(400, "invalid_order", "Missing or invalid item selections.");
    }
    ...
    let pricing;
    let junkSelections: GeneralJunkSelection[];
    try {
      junkSelections = payload.items as GeneralJunkSelection[];
      pricing = calculateGeneralJunkRemovalPrice({
        items: junkSelections,
    ...
    const orderedItems = junkSelections
      .filter((selection) => selection.quantity > 0)
      .map((selection) => {
        const config = GENERAL_JUNK_ITEMS[selection.itemKey as GeneralJunkItemKey];
        return {
    ...
    try {
      await saveGeneralJunkRemovalOrder(env.ORDERS_KV, record);
    } catch {
  ...
  try {
    syncResult = await syncGeneralJunkRemovalOrderToJobber(env, record);
  } catch (cause) {
    syncResult = {
      ok: false,
  ...
  const { onlineBookingUrl } = await finalizeSyncedOrder(env, {
    serviceLabel: "General Junk Removal",
    orderId: record.id,
